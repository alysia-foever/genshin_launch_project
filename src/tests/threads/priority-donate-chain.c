...
void test_priority_donate_chain(void) {
    ...
    for (i = 1; i < NESTING_DEPTH; i++) {
        ...
        msg("%s should have priority %d.  Actual priority: %d.", thread_name(), thread_priority,
            thread_get_priority());
...
static void donor_thread_func(void *locks_) {
    ...
    msg("%s should have priority %d. Actual priority: %d", thread_name(), (NESTING_DEPTH - 1) * 3,
        thread_get_priority());
